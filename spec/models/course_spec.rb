...
describe Course do
  ...
  context '#validate_then_save' do
    it 'returns false if the name is empty' do
      ...
      expect(course.custom_errors).to include('Course cannot be posted without a name')
    end
    it 'returns false if the start_date is empty' do
      ...
      expect(result).to be_falsey
      expect(course.custom_errors).to include('Course needs a start date')
    end
    it 'returns false if the end_date is empty' do
      ...
      expect(result).to be_falsey
      expect(course.custom_errors).to include('Course needs an end date')
    end
    it 'returns false if the start date is in the past' do
      ...
      expect(result).to be_falsey
      expect(course.custom_errors).to include('Course must start in the future')
    end
    it 'returns false if the start date is after the end date' do
      ...
      expect(result).to be_falsey
      expect(course.custom_errors).to include('Course must end after it starts!')
    end
