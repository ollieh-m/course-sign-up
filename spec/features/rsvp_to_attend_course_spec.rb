...
feature 'Someone rsvps with their email address to attend a course' do
  ...
  scenario 'and the course has that attendee as the only unconfirmed attendee' do
    within(:css, 'nav') do
      click_on('Courses')
    end
    click_on('RSVP')
    ...
    click_on('testname')
    expect(page).to have_css('.attendees li.unconfirmed', text: 'attendee@email.com')
  end
...
#if someone signs up again with the same details, the same attendee is made to rsvp - if the details are different, a new attendee is created
#attendees can be identified by their email and date of birth - if either is new it's a new user, 
#otherwise the same attendee is used (with their name updated if it is different...)
...
#the token is generated by the rsvp, isn't saved in the database, and can only be known by whoever gets the email - it is like the password for logging into the rsvp, a password that no-one can know unless they get the email
